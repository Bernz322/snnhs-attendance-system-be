 ...
 * Created token expires in 12 hour.
 * @param {Object} user 
 * @returns {Token} token
 */
const createToken = (user) => {
    return token = sign({ id: user.id }, process.env.ACCESS_TOKEN_SECRET, { expiresIn: '12h' })
}
...
const validateToken = async (req, res, next) => {
    ...
    if (req.headers.authorization && req.headers.authorization.startsWith('Bearer')) {
        try {
            ...
            res.status(403).json({message:"Token is expired. Please re-login."}) // If something goes wrong, return 403 meaning, token is invalid 
        }
    }
